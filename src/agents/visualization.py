...
class AgentNetworkVisualizer:
    ...
    def update_network(self, agent_system):
        ...
        def add_agent_to_graph(agent, parent_id=None):
            G.add_node(agent.id, role=agent.role.value, tasks=len(agent.tasks))
            if parent_id:
    ...
    def update_reconciliation(self, reconciliation_data: Dict):
        ...
        for round_num, round_data in enumerate(reconciliation_data["rounds"], 1):
            round_id = self.tree.insert(
                "", "end", text=f"Round {round_num}", values=("", "", "")
            )
            ...
            for agent_id, result in round_data["results"].items():
                ...
                self.tree.insert(
                    ...
                    "end",
                    text="",
                    values=(agent_id, json.dumps(result["data"], indent=2), status),
                )
...
class ActivityVisualizer:
    def __init__(self, master):
        ...
        self.agent_combo = ttk.Combobox(filter_frame, textvariable=self.agent_var)
        ...
        self.activity_combo = ttk.Combobox(filter_frame, textvariable=self.activity_var)
    ...
    def apply_filters(self, event):
        ...
        for i, activity in enumerate(self.activities, 1):
            ...
            if (
                activity_filter != "All"
                and activity["activity_type"] != activity_filter
            ):
                continue
