...
class AgentManagerUI:
    ...
    def create_control_tab(self):
        ...
        mode_frame = ttk.LabelFrame(
            control_frame, text="System Configuration", padding="5"
        )
        ...
        self.status_frame = ttk.LabelFrame(
            control_frame, text="System Status", padding="5"
        )
    ...
    def create_task_tab(self):
        ...
        # Complexity
        ttk.Label(basic_frame, text="Task Complexity:").grid(
        ...
        scope_scale = ttk.Scale(
            basic_frame, from_=1, to=10, variable=self.scope_var, orient=tk.HORIZONTAL
        )
        scope_scale.grid(row=1, column=1, padx=5, pady=2, sticky=(tk.W, tk.E))

        # Urgency
        ttk.Label(basic_frame, text="Task Urgency:").grid(
            row=2, column=0, padx=5, pady=2
        )
        self.urgency_var = tk.IntVar(value=1)
        urgency_scale = ttk.Scale(
            basic_frame, from_=1, to=10, variable=self.urgency_var, orient=tk.HORIZONTAL
        )
        urgency_scale.grid(row=2, column=1, padx=5, pady=2, sticky=(tk.W, tk.E))

        # Advanced Parameters
        advanced_frame = ttk.LabelFrame(
            task_frame, text="Advanced Parameters", padding="5"
        )
        advanced_frame.pack(fill=tk.X, padx=5, pady=5)

        # Task Type
        ttk.Label(advanced_frame, text="Task Type:").grid(
            row=0, column=0, padx=5, pady=2
        )
        ...
        # Priority
        ttk.Label(advanced_frame, text="Priority:").grid(
        ...
        # Timeout
        ttk.Label(advanced_frame, text="Timeout (s):").grid(
            row=2, column=0, padx=5, pady=2
        )
        self.timeout_var = tk.IntVar(value=60)
        timeout_spin = ttk.Spinbox(
            advanced_frame, from_=1, to=3600, textvariable=self.timeout_var
        )
        ...
        # Update button
        ttk.Button(task_frame, text="Update Preview", command=self.update_preview).pack(
            pady=5
        )
    ...
    def create_status_display(self):
        ...
        if self.agent_system:
            ...
            scrollbar = ttk.Scrollbar(
                self.status_frame, orient=tk.VERTICAL, command=self.tree.yview
            )
            ...
            ttk.Label(self.status_frame, text="Hierarchical Management Disabled").pack(
                padx=5, pady=5
            )
    ...
    def update_preview(self):
        ...
        if not self.agent_system:
            ...
            resources = asyncio.run(self.agent_system.director.analyze_task(task))
