...
class AgentLogger:
    ...
    def setup_file_logger(self):
        ...
        file_handler = logging.FileHandler(
            self.log_dir
            / f"agent_activity_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def get_activities(
        ...
        if start_time:
            filtered = [
                a for a in filtered if datetime.fromisoformat(a.timestamp) >= start_time
            ]

        if end_time:
            filtered = [
                a for a in filtered if datetime.fromisoformat(a.timestamp) <= end_time
            ]
    ...
    def export_activities(self, filepath: Optional[str] = None) -> str:
        ...
        if not filepath:
            filepath = (
                self.log_dir
                / f"activities_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
